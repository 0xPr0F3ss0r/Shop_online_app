...
class AuthService extends GetxController {
  ...
  try {
    ...
    if (googleUser == null) {
      ...
      Get.snackbar("Error", "Google Sign-In was cancelled", colorText: Colors.red, duration: Duration(seconds: 2));
      return;
    }
    ...
    if (name != null) {
      ...
      SecondName = nameParts.length > 1 ? nameParts.sublist(1).join(' ') : ''; // Join remaining parts as last name
      //uid = FirebaseAuth.instance.currentUser!.uid;
    }
    ...
    Get.snackbar("Error", "Error during Google Sign-In: ${e.toString()}", colorText: Colors.red, duration: Duration(seconds: 2));
  }
}
  ...
  Future<bool> loginUserWithEmailAndPassword(String email, String password) async {
    try {
      ...
      Get.snackbar("Error", "Email or Password incorrect", colorText: Colors.red, barBlur: 10.0);
      return false;
    }
  }
